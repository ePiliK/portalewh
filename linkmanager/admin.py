...
class LinkAdmin(admin.ModelAdmin):
    ...
    def get_list_filter(self, request):
        ...
        # Remove 'client' from list_filter for non-superuser
        return [f for f in super().get_list_filter(request) if f != 'client']
    ...
    def import_csv(self, request):
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                # Verifica che tutti i clienti nel file CSV esistano già nel database
                missing_clients = []
                for row in csv_reader:
                    if not Client.objects.filter(name=row['client']).exists():
                        missing_clients.append(row['client'])
                
                if missing_clients:
                    self.message_user(request, "Il file CSV contiene clienti non censiti: " + ", ".join(missing_clients), level='ERROR')
                    return redirect("..")

                # Riposiziona il lettore CSV all'inizio del file
                csv_file.seek(0)
                decoded_file = TextIOWrapper(csv_file.file, encoding='utf-8-sig')
                csv_reader = csv.DictReader(decoded_file)
                ...
                for row in csv_reader:
                    try:
                        client = Client.objects.get(name=row['client'])
                        Link.objects.create(
                            client=client,
